...
export class Game {
                        ...
                        try {
                            ...
                            if (messages.length > 0) {
                                for (const msg of messages) {
                                    ...
                                    const isDuplicate = (this.wolfChatHistory || []).some(m =>
                                        m.from === player.name && m.content.trim() === trimmedMsg
                                    );
    ...
    startVote() {
        ...
        // 开始投票倒计时
        this.startCountdown(this.config.game.voteTime, () => {
            this.resolveVote();
        });
    }
    ...
    getGameStateForAI(playerId) {
        ...
        return {
            gameState: {
                ...
                speakingOrder: this.speakingOrder ? this.speakingOrder.map(id => ({
                    id: reverseIdMap[id],
    ...
    async resumeGameLoop() {
        ...
        console.log(`[Resume] 恢复游戏循环，阶段: ${this.currentPhase}, 当前发言者: ${this.currentSpeakerId}`);
        ...
        if (this.currentPhase === 'day' && this.currentSpeakerId && this.speakingOrder) {
            ...
            if (currentSpeakerIndex !== -1) {
                // 从当前发言者的下一个人继续
                await this.continueSpeakingFrom(currentSpeakerIndex + 1);
            }
        } else if (this.currentPhase === 'vote') {
    ...
    // 从指定索引继续发言
    async continueSpeakingFrom(startIndex) {
    ...
    async processRemainingNightActions() {
        // 检查是否所有需要行动的玩家都已行动
        const aliveSpecialRoles = this.getAlivePlayers().filter(([id, p]) =>
            p.role && ['狼人', '预言家', '女巫', '守卫'].includes(p.role.name) && p.isAI
        );
